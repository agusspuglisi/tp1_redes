            ...
            while True:
                ...
                try:
                    ...
                    if ack_packet.ack and ack_packet.seq_num == seq_num:
                        if attempts > 1:
                            logging.debug(f"ACK received for packet {seq_num} after {attempts} attempts")
                        seq_num = 1 - seq_num
                        break
                except socket.timeout:
                    retransmissions += 1
                    if attempts <= 3: 
                        logging.warning(f"Timeout occurred, retransmitting packet {seq_num} (attempt {attempts})")
                    continue
